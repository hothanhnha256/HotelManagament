...
export interface OrderRoomProps {
  id: string;
  date: string;
  customerName: string;
...
export interface OrderRoomInterface {
  id: string;
  onClose: () => void;
}
const fakeDataOrders: OrderRoomProps = {
  id: "1",
  date: "2021-09-01",
...
export default function OrderRoomDetail(props: OrderRoomInterface) {
  const [order, setOrder] = useState<OrderRoomProps | null>(null);
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg w-full max-w-md">
        ...
        {isLoading ? (
          ...
          <div>No oOrderRooms available</div>
        )}
      </div>
    </div>
  );
