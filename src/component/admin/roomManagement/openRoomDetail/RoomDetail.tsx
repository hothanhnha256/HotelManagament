...
export default function RoomDetail(props: RoomInterface) {
  ...
  //FOR SELECT ID HOUSEKEEPER

  const [housekeeperList, setHousekeeperList] = useState<EmployeeProps[]>([]);
  const [roomInChargePerson, setRoomInChargePerson] = useState<EmployeeProps[]>(
    []
  );
  const handleFetchRoomInChargePerson = async () => {
    try {
      const response = await fetch(
        `${APIURL}/employees/all?role=ROOM EMPLOYEE`
      );
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
      const result = await response.json();
      setRoomInChargePerson(result.data);
    } catch (error) {
      //console.log("Failed to fetch Room In Charge Person list ", error);
    }
  };
  const handleFetchHousekeeper = async () => {
    try {
      const response = await fetch(`${APIURL}/employees/all?role=HOUSE KEEPER`);
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
      const result = await response.json();
      ...
      //console.log("Failed to fetch Housekeeper list ", error);
    }
  };

  useEffect(() => {
    handleFetchHousekeeper();
  }, []);
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <div className="text-black dark:text-white">
          ...
          <p>
            ...
            {roomTypeMapping(props.entry?.LoaiPhong || "")}
          </p>
          <p>
        ...
        <form onSubmit={handleUpdate}>
          ...
          <div className="mb-4">
            ...
            <select
              ...
              required
            >
              <option value={newRoom.housekeeperId}>
                {
                  housekeeperList?.find(
                    (employee) => employee.ID === newRoom.housekeeperId
                  )?.HoTen
                }
              </option>
              {housekeeperList.map((employee) => (
                <option key={employee.ID} value={employee.ID}>
                  {employee.HoTen} {employee.CCCD}
                </option>
              ))}
            </select>
          </div>
          ...
          <div className="mb-4">
            ...
            <select
              ...
              className="w-full p-2 border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 text-black dark:text-white"
              required
              onClick={handleFetchRoomInChargePerson}
            >
              <option value={newRoom.roomInChargePersonID}>
                {
                  roomInChargePerson?.find(
                    (employee) => employee.ID === newRoom.roomInChargePersonID
                  )?.HoTen
                }
              </option>
              {roomInChargePerson.map((employee) => (
                <option key={employee.ID} value={employee.ID}>
                  {employee.HoTen} {employee.CCCD}
                </option>
              ))}
